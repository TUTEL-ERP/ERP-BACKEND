...
namespace server.Repository
{
    public class UserSetupRepository : IUserSetupRepository
    {
        ...
        public UserSetupRepository(ApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<DataTable> ExecuteProcedureAsync(string usrname, UserAction action, UserRequestDto request)
        {
            using var connection = new SqlConnection(_context.Database.GetConnectionString());
            using var cmd = new SqlCommand("SP_USER_SETUP", connection);
